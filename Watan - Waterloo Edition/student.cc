...
bool Student::achieveAttempt() {
  ...
  Goal *goal = this->watan->getGoal(goalIndex);

  if(!this->canAfford(goal->goalCost)){
  ...
  this->loseResources(goal->goalCost);
...
void Student::moveGeese() {
  ...
  while(true) {
    ...
    while (stealChoices.empty()) {
      ...
      stealChoices = this->watan->attemptPlaceGeese(this->colour, tileIndex);
    }
