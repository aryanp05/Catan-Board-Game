...
// Observer for GoalState, inheriting from Observer base class
class GoalObserver : public Observer<GoalState> {
public:
    virtual void notify(Subject<GoalState> &whoFrom) = 0; // Notify method for GoalState
    virtual ~GoalObserver() = default; // Destructor
};
