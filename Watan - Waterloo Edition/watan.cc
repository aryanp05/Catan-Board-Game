...
Watan::~Watan() {
  ...
  std::string Watan::generateTileString(unsigned int seed) const {
      ...
      // Resource types: CAFFEINE(0), LAB(1), LECTURE(2), STUDY(3), TUTORIAL(4), NETFLIX(5)
      ...
      for (int i = 0; i < 19; ++i) {
          ...
          if (resource == 5) { // NETFLIX: append 7 probability
              board_str += "7 ";
    ...
    for (auto s : this->students) {
      ...
      if (resourceCount >= 10) { // if exceeded, randomly remove a resource till half are removed
        ...
        while (half > 0) {
          ...
          Resource randomResource = rInt == 0 ? Resource::CAFFEINE 
                              : rInt == 1 ? Resource::LAB 
                              : rInt == 2 ? Resource::LECTURE 
                              : rInt == 3 ? Resource::STUDY 
                              : rInt == 4 ? Resource::TUTORIAL 
                              : Resource::NETFLIX;
...
Goal* Watan::getGoal(int goalIndex) {
  return this->board->getGoalList().at(goalIndex);
}
...
const Colour curStudent) const {
  ...
  std::vector<Goal *> goals = this->board->getGoalList();
  ...
  for (Colour t: this->turnOrder) {
    ...
    line += std::to_string(resArray.getResourceAmount(Resource::CAFFEINE));
    line += " " + std::to_string(resArray.getResourceAmount(Resource::LAB));
    line += " " + std::to_string(resArray.getResourceAmount(Resource::LECTURE));
    line += " " + std::to_string(resArray.getResourceAmount(Resource::STUDY));
    line += " " + std::to_string(resArray.getResourceAmount(Resource::TUTORIAL));
    line += " g";
    for (Goal *&g : goals) {
      if (g->getOwner() == t) {
    ...
    for (size_t i = 0 ; i < crits.size() ; i++) {
      if (crits.at(i)->getOwner() == t) {
        ...
        switch (crits.at(i)->getTier()) {
          case CritType::Assignment: line += " 1"; break;
          case CritType::Midterm: line += " 2"; break;
